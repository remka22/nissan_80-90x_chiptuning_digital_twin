...
LABELS = [
    # ====================== ПАРАМЕТРЫ ======================
    # ЕДИНИЦА — КОРОТКАЯ. Всё пояснение уходит в HINTS (тултип), иначе подпись в
    # таблице разрастается и читать колонку значений невозможно.
    (0x140A, "Обороты",              16, "об/мин",             12.807),
    (0x1482, "Нагрузка (ось карт)",   8, "у.е.",               None),
    (0x1413, "Наполнение мгновенное", 8, "у.е.",               None),  # $1413−$1482 = газовка
    (0x14A2, "Дроссель открытие",     8, "у.е.",               None),  # % считается отдельно из $1492: (ацп−мин)/344
    (0x004D, "Впрыск, мс",           16, "мс",                 0.010),  # $004D→UPP-каналы, ТИК 10 мкс (испр. 04.08.26)
    (0x142C, "Мёртвое время форсунки",8, "у.е.",               None),
    (0x1450, "Обогащение ускорения",  16, "у.е.",              None),   # >0 = было обогащение
    (0x140F, "УОЗ сырьё",             8, "у.е.",               None),
    (0x143B, "УОЗ из карты",          8, "°",                  None),
    ...
    (0x14EA, "Поправка угла по детонации, °", 8, "°", None),
    (0x00F7, "Давление, кПа",         8, "кПа",                None),
    # Расход — то, что прослойка ДАД отдаёт заводской цепочке (врезка 8965). Логируем
    # старший байт: 0 значит упёрлись в нижний заводской зажим 124, 255 — в верхний
    # 65414. Рабочий диапазон примерно 4 на холостом и 117 на верхах.
    (0x144B, "Расход (прослойка)",    8, "у.е.",               None),
    (0x00FA, "VE (ДАД)",    8, "×",     0.0078125),  # карта 0x4900
    (0x00FB, "Ktps (ДАД)",  8, "×",     0.0078125),  # карта 0x4B00
    (0x1431, "Коррекция по лямбде",   8, "×",                  0.01),
    (0x17F8, "Живая карта",           8, "у.е.",               None),
    (0x17F9, "Состояние указателя",    8, "у.е.",              None),
    # ======================== АЦП (сырьё) ===================
    (0x1408, "АЦП Расходомер/ДАД", 16, "у.е.",              None),
    (0x00B1, "Индекс температурных таблиц", 8, "у.е.",  None),  # с v35 вместо сырья дросселя
    (0x1402, "РХХ / контур холостого",      8, "у.е.",  None),  # с v35
    (0x008F, "АЦП Напряжение борта",8,"у.е.",              None),
    (0x004C, "АЦП Температура ОЖ",8, "у.е.",              None),
    (0x1400, "АЦП Лямбда",        8, "у.е.",              None),
    ...
    (0x17E0, "АЦП канал 9 (свободный)", 8, "у.е.",         None),
]

# Строки, которые имеют смысл ТОЛЬКО на прошивке с ДАД. На расходомерной их не
# показываем совсем (см. snapshot): в этих ячейках лежит мусор, а не ноль.
DAD_ONLY_ADDRS = {"$00F7", "$144B", "$00FA", "$00FB"}

# Пояснения к строкам левой таблицы — ТОЛЬКО в тултип. В самой таблице стоит короткая
# единица измерения, потому что длинные приписки («сырьё (/8=TP)», «в единицах впрыска»)
# занимали всю ширину колонки и ничего при этом не объясняли.
HINTS = {
    0x140A: "16 бит, множитель 12.807. Масштаб не закрыт — см. базу знаний.",
    0x1482: "Сглаженное наполнение, по нему берётся столбец карт. TP = значение / 8.",
    0x1413: "Наполнение до сглаживания. Разница с нагрузкой — это перегазовка.",
    0x14A2: "Открытие дросселя в отсчётах от выученного нуля. Проценты считаются отдельно из АЦП $1492.",
    0x004D: "Реальная длительность впрыска, отданная в UPP. Цена тика 10 мкс.",
    0x142C: "Мёртвое время форсунки в единицах впрыска, прибавляется после всех множителей.",
    0x1450: "Добавка топлива на тип-ине, ПРЯМО в длительность впрыска (8a8b). Произведение трёх таблиц: 0x7900 по скорости открытия × 0x7910 по положению × 0x7BC0 по температуре. Ноль — дроссель не движется.",
    0x140F: "Сырьё угла. Градусы = 70 − байт.",
    0x143B: "Угол из карты, приблизительно, до коррекций.",
    0x14EA: "Плюс — блок добавил угол, минус — снял. Ноль — поправки нет.",
    0x00F7: "Абсолютное давление во впуске. Считает наша рутина ДАД.",
    0x144B: "Старший байт расхода, который прослойка отдаёт заводской цепочке. 0 или 255 — упёрлись в заводской зажим.",
    0x00FA: "Коэффициент наполнения из карты VE. 1.00 — поправки нет.",
    0x00FB: "Поправка по дросселю из карты Ktps. 1.00 — поправки нет.",
    0x1431: "Коррекция по лямбде. 1.00 — поправки нет. У этой машины лямбда молчит.",
    0x17F8: "0 нет / 1 смесь / 2 угол / 3 VE / 4 Ktps.",
    0x17F9: "0 карты из ПЗУ / 1 карты из тени / 255 указатель испорчен.",
    0x1408: "Сырьё АЦП, 10 бит, прижато вправо.",
    0x00B1: "По нему блок выбирает ячейку ВО ВСЕХ температурных таблицах: пусковой 0x7E20, прогревочных 0x7EB0/0x7F30, температурной поправке накачки 0x7BC0. Считается из $140C через ограничитель скорости, шкала перевёрнута относительно сырья ОЖ.",
    0x1402: "Переменная контура холостого хода. Пишется на b1cd, читается заводским кодом в шести местах зоны управления ХХ, сравнивается с АЦП канала 4. Роль опознана по коду, абсолютный смысл не выведен.",
    0x008F: "Напряжение борта. 175 примерно соответствует 14 В (оценка).",
    0x004C: "Температура охлаждающей жидкости. Термистор: меньше значение — горячее.",
    0x1400: "Узкополосный датчик кислорода. У этой машины стабильно ноль.",
    0x17E0: "Свободный канал АЦП, завод его не опрашивает.",
}
...
NARROW_MAP = [
    ...
    0x00B1,           # с v35: индекс температурных таблиц (был 0x1492 — сырьё дросселя,
                      # дубль $14A2 со связью 0.9999, в панели больше не используется)
    0x1402,           # с v35: переменная контура ХХ (был 0x1493)
    0x140F, 0x143B,
    ...
    0x142C,
    0x14A2,
    0x1450,           # ⚠ с v34: СТАРШИЙ байт добавки обогащения ускорения.
                      # Раньше в этом слоте лежал $14DE — и это была ЧУЖАЯ величина:
                      # она уходит в асинхронный впрыск (8810/884b), а к длительности
                      # основного впрыска прибавляется именно $1450 (8a8b). Крутили одно,
                      # смотрели на другое.
    0x00FA, 0x00FB,   # VE/Ktps выбранные (ДАД) — с v27 кэш в $FA/$FB
    0x004D, 0x004E,   # реальный впрыск
    0x17F8, 0x17F9,   # карта в тени / вердикт
    0x1451,           # с v34: МЛАДШИЙ байт добавки (был $141A — смесь из карты,
                      # всего 6 разных значений за лог и выводится из карты + ячейки)
    0x14EA,           # откат по детонации (128 = нет)
    0x144B,           # расход от прослойки ДАД, старший байт (см. колонку ниже)
]
...
def pick_bin_dialog(start_dir=""):
    """Родное окно выбора файла macOS. → (путь, ошибка).

    Выпадашка со списком не годилась: она показывает ТОЛЬКО логер/ и j30/, а прошивки
    лежат где попало, и выбрать файл из другой папки было нечем. Диалог поднимаем
    через osascript — панель локальная, браузер путь к файлу отдать не может в принципе.
    """
    import subprocess
    d = start_dir if (start_dir and os.path.isdir(start_dir)) else HERE
    scr = ('tell application "System Events" to activate\n'
           'set f to choose file with prompt "Выбери прошивку (.bin, 32 КБ)" '
           'default location POSIX file "%s"\n'
           'POSIX path of f' % d.replace('"', '\\"'))
    try:
        r = subprocess.run(["osascript", "-e", scr], capture_output=True, timeout=180)
    except Exception as e:
        return "", "не открылось окно выбора: %s" % e
    p = (r.stdout or b"").decode("utf-8", "replace").strip()
    if not p:
        err = (r.stderr or b"").decode("utf-8", "replace").strip()
        # -128 = пользователь нажал «Отмена». Это не ошибка, молча ничего не меняем.
        return "", "" if ("-128" in err or not err) else err
    if not os.path.exists(p):
        return "", "файла нет: %s" % p
    if os.path.getsize(p) != 32768:
        return "", "это не прошивка ЭБУ: размер %d байт, нужно 32768" % os.path.getsize(p)
    return p, ""
...
def load_tables(path):
    ...
    # ДАД speed-density. ⚠ 09.08.26 врезка ПЕРЕЕХАЛА с 89D8 на 8965 — на выход
    # заводской таблицы линеаризации. Детект только по старому адресу молча прятал
    # карты VE и Ktps, и страницу настройки ДАД было не открыть. Проверяем оба места:
    # 8965 (файл 0x0965) — текущая схема, 89D8 — прежняя, чтобы старые бины открывались.
    hooked = ((b[0x0965] == 0xBD and b[0x0966] == 0xC7 and b[0x0967] == 0x00)
              or (b[0x09D8] == 0xBD and b[0x09D9] == 0xC7 and b[0x09DA] == 0x00))
    # ⚠ ВРЕЗКИ МАЛО. Первая команда рутины — TST $CA13 (файл 0x4A13): ноль = хвостовой
    # переход в заводскую таблицу расходомера, и до карт VE/Ktps дело не доходит вообще.
    # Пока детект смотрел только на врезку, панель на прошивке с режимом 0 показывала
    # карты VE и Ktps и разрешала обучение — то есть предлагала настраивать то, что блок
    # в этом режиме не читает. Режим — часть признака, а не отдельная мелочь.
    t["dad_hook"] = hooked                  # прошивка УМЕЕТ в ДАД
    t["dad_mode"] = b[0x4A13] if hooked else 0
    dad = hooked and b[0x4A13] != 0         # прошивка СЕЙЧАС РАБОТАЕТ по ДАД
    t["dad"] = dad
    if dad:
        t["ve"] = rd(0x4900, 256)       # VE 16×16 (значение/128 = наполнение)
        # ⚠ ОСЬ ОБОРОТОВ У НАШИХ КАРТ СВОЯ, 0x4A20 (09.08.26). Раньше здесь стояла
        # заводская 0x7B20, и после переезда карт 12 строк из 16 подписывались чужими
        # оборотами: панель писала 1800 там, где на самом деле 2000, и 6000 вместо 6400.
        # Хуже подписи: по этой же оси считается подсветка и РАСПРЕДЕЛЕНИЕ ЗАМЕРОВ ПРИ
        # ОБУЧЕНИИ — поправка уезжала в соседнюю строку.
        # Для старых бинов (до 09.08.26) карты сидят на заводской оси, а 0x4A20 пустой
        # (0x3F) — тогда откатываемся на 0x7B20.
        _rax = rd(0x4A20, 16)
        t["ve_rax"] = _rax if any(z != 0x3F for z in _rax) else rd(0x7B20, 16)
    ...
    if dad and any(z != 0x3F for z in rd(0x4B00, 16)):
        ...
        t["ktps_rax"] = t["ve_rax"]     # обороты — ОБЩАЯ ось с VE (см. выше)
...
def _log_sampler(stop_ev):
    ...
    while not stop_ev.is_set():
        ...
        if fr != last and fr > 0:
            ...
            try:
                row = _log_row()
                # ФАЙЛ СОЗДАЁТСЯ ТОЛЬКО ЗДЕСЬ — на первой реальной строке. Раньше он
                # открывался вместе с подключением, и каждое подключение «на посмотреть»
                # оставляло в логи/ пустышку с одной шапкой. Разгребать их приходилось руками.
                if LOGST["f"] is None:
                    LOGST["f"] = open(LOGST["path"], "w", encoding="utf-8-sig")
                    LOGST["f"].write(";".join(LOG_HEADER) + "\n")
...
def log_start():
    ...
    # имя выбрано, но на диск ничего не пишем: файл заведёт сборщик, когда придёт строка
    LOGST.update(on=True, path=fn, n=0, f=None, t0=time.time())
...
def log_stop():
    ...
    try:
        if LOGST["f"] is not None: LOGST["f"].close()
    except Exception: pass
    if LOGST["f"] is None: LOGST["path"] = ""      # ни одной строки — файла и нет
    LOGST["on"] = False; LOGST["stop"] = None; LOGST["thread"] = None; LOGST["f"] = None
    return True
...
def reader_loop(ser, stop_ev, fname):
    ...
    while not stop_ev.is_set():
        try:
            ...
            # Сообщение видит пользователь — пишем по-человечески, без слова «исключение»
            # и без питоновского текста ошибки первым словом.
            with LOCK: STATE["error"] = "Связь с блоком потеряна (%s). Выбери порт заново." % e
            break
        if chunk:
            with DUMP_LOCK:
...
def _floor_idx(val, axis):
    """Номер ячейки БЕЗ интерполяции — по правилу заводского $80CF (80ec-80f8):
    наименьший i, при котором значение меньше следующей точки оси."""
    n = len(axis)
    for i in range(n - 1):
        if val < axis[i + 1]: return i
    return n - 1


def _cells_for(rpm_ax, col_ax, rpm, col, col_interp=True):
    """Ячейки с весами — ровно так, как их смешивает ЭБУ.
    col_interp=False для Ktps: прошивка берёт столбец по дросселю БЕЗ интерполяции
    (ось январская, неравномерная, там простой поиск), поэтому размазывать замер на
    соседний столбец нельзя — он бы портил ячейку, которую блок вообще не читал."""
    out = []
    cw = _bilin(col, col_ax) if col_interp else [(_floor_idx(col, col_ax), 1.0)]
...
def learn_feed(now, ram, top, t, afr_fact):
    ...
    # ⚠ с v34 признак «сейчас идёт обогащение ускорения» берём из $1450 (16 бит) —
    # это ТА добавка, что реально прибавляется к впрыску. До v34 сюда подавался $14DE,
    # который относится к асинхронному впрыску: обучение считало режим установившимся
    # там, где топливо на самом деле накачивалось, и портило ячейки.
    acc_now = ((ram.get(0x1450, 0) << 8) | ram.get(0x1451, 0)) if 0x1450 in ram else None
    bad = _gate(rpm, tps, ram.get(0x004C), acc_now, inj_zero,
                afr_fact, now - LCUT["t"])
    ...
    cells = _cells_for(rpm_ax, list(cax), rpm, col, col_interp=(which == "ve"))
...
def _bake_bin(maps, suffix="_лог_"):
    ...
    try:
        ...
        # НИЧЕГО НЕ ИЗМЕНИЛОСЬ — файл не плодим. Раньше каждая остановка клала в логи/
        # ещё одну побайтно точную копию выбранной прошивки, и в списке прошивок
        # накапливались близнецы, среди которых легко прошить не тот файл.
        if bytes(rom) == src:
...
class NetSerial:
    ...
    def read(self, n=1):
        ...
        try:
            ...
            return b""                      # просто пауза между кадрами — это норма
        except OSError as e:
            self.is_open = False
            raise IOError("связь с мостом оборвалась: %s" % e)
        # ⚠ ПУСТОЙ ОТВЕТ У СОКЕТА — ЭТО НЕ «ТИШИНА», А КОНЕЦ СВЯЗИ. Пока он глотался
        # как «данных нет», reader_loop крутился вечно на мёртвом сокете: ошибки нет,
        # признак «подключено» не снимается, значок висит жёлтым, и вдобавок пустой recv
        # возвращается мгновенно — цикл без пауз ел ядро процессора.
        if not b:
            self.is_open = False
...
PING_BYTE = 0x55      # «панель здесь» для моста ESP32 (см. esp32/j30_мост_ЭБУ-ШДК)
# ⚠ ПЕРИОД 2 СЕКУНДЫ, А НЕ ОДНА — И ЭТО НЕ ВКУСОВЩИНА.
# В ЭБУ есть автоматический ресинхрон приёма: автомат, застрявший посреди недобранной
# команды (потеряли байт от помехи), сбрасывается после ~1.1 с ТИШИНЫ — 100 пустых
# проходов задачи, счётчик $17EA. ЛЮБОЙ пришедший байт этот счётчик обнуляет
# (build_targeted_patch ИИ.py, метка DUMP). Пинг раз в секунду обнулял его РАНЬШЕ порога,
# и ресинхрон не сработал бы никогда: блок остался бы подвисшим до перезапуска зажигания.
# Ломало это ОБА канала, не только мост.
# 2 с оставляют окно тишины длиннее 1.1 с. Мост держит проводной режим 4 с (USB_TTL_MS),
# так что двух секунд ему хватает с запасом. На поток телеметрии период не влияет никак —
# пинг идёт в обратную сторону, кадры от ЭБУ идут своим чередом.
PING_PERIOD = 2.0


def _keepalive_loop(ser, stop_ev):
    """Раз в PING_PERIOD секунд шлём мосту служебный байт: «панель на этом канале, я живая».

    ЗАЧЕМ. Мост определяет, что панель сидит НА ПРОВОДЕ, только по входящим байтам —
    других способов узнать, открыт ли USB-порт на компе, у платы нет (линии DTR/RTS
    заняты схемой автосброса, и панель их намеренно не трогает). Получив этот байт по
    USB, мост уходит в проводной режим: рвёт сетевого клиента и отдаёт всё в провод.

    БЕЗОПАСНОСТЬ. Мост байт съедает, в блок он не уходит. Если же провод воткнут в ЭБУ
    НАПРЯМУЮ, без моста, — блок в состоянии покоя неизвестный байт игнорирует (автомат
    приёма: все сравнения мимо, состояние не меняется). В середину чужой команды байт
    попасть не может: шлём под SER_LOCK, тем же замком, что держат дамп и заливка.
    """
    while not stop_ev.is_set():
        stop_ev.wait(PING_PERIOD)
        if stop_ev.is_set(): break
        # ждём замок с таймаутом: во время долгого дампа пинги просто пропускаем,
        # а не копим очередь потоков на входе
        if not SER_LOCK.acquire(timeout=0.5): continue
        try:
            if CTRL.get("ser") is ser:
                ser.write(bytes([PING_BYTE]))
        except Exception:
            return                       # порт умер — молча уходим, диагностику даст reader_loop
        finally:
            SER_LOCK.release()
...
def do_start(port, baud):
    ...
    with SER_LOCK, CTRL_LOCK:
        ...
        th.start()
        # тот же stop_ev: отключение гасит и пинг
        threading.Thread(target=_keepalive_loop, args=(ser, stop_ev), daemon=True).start()
...
def snapshot():
    ...
    _flv = [{"name": n, "addr": "$17FF" if n in [x[1] for x in FLAGBITS] else "$0053",
             "val": _fl.get(n), "unit": "0/1", "real": None, "hint": ""} for n in FLAGNAMES]
    ...
    top = {
        ...
        # ТЕКУЩЕЕ VE — то, что блок реально применил на этом обороте ($00FA, 128 = 1.00).
        # Это, а не «поправка факт/цель», нужно в верхней строке: поправка — производная
        # величина, которая без ШДК пустая и на неё всё равно нельзя посмотреть и что-то понять.
        "ve": round(ram[0x00FA] / 128.0, 3) if 0x00FA in ram else None,
        "ktps": round(ram[0x00FB] / 128.0, 3) if 0x00FB in ram else None,
    }
    ...
    # ГАЗ %: из $14A2 — ЭТО ОТКРЫТИЕ, КОТОРОЕ СЧИТАЕТ САМ БЛОК от ВЫУЧЕННОГО ИМ нуля.
    # Раньше считалось из сырья $1492 минус «самое малое, что панель увидела за сессию» —
    # и врало: в логе 11.08 $14A2 доходил до 142, а панель показывала максимум 84.6%.
    # Полный ход = 172 (конец оси дросселя 0x4C00, по ней блок сам индексирует Ktps).
    tps_op = ram.get(0x14A2)
    if tps_op is not None:
        top["tps_pct"] = max(0.0, min(100.0, round(tps_op / 172.0 * 100, 1)))
    else:
        top["tps_pct"] = None
    t = load_tables(SEL["bin"])
    fuel_out = ign_out = ve_out = ktps_out = None
    top["is_dad"] = bool(t and t.get("dad"))
    # Прошивка УМЕЕТ в ДАД, но переключена в расходомер ($4A13 = 0) — это надо сказать
    # словами, иначе исчезнувшие карты VE/Ktps выглядят как поломка панели.
    top["dad_off"] = bool(t and t.get("dad_hook") and not t.get("dad"))
    # НА РАСХОДОМЕРНОЙ ПРОШИВКЕ ЭТИХ ВЕЛИЧИН НЕ СУЩЕСТВУЕТ. Рутина ДАД в режиме
    # расходомера уходит в заводскую таблицу первой же командой и в свои ячейки
    # zero-page не пишет вообще — там лежит что попало. Показывать это как «давление»
    # и «VE» нельзя: числа выглядят настоящими.
    if not top["is_dad"]:
        d["vars"] = [v for v in d["vars"] if v["addr"] not in DAD_ONLY_ADDRS]
    if t:
        ...
        if t.get("ktps") is not None:
            ...
            if rpm_raw is not None and a2 is not None:
                ...
                # столбец Ktps — ИМЕННО floor, как в прошивке: ближайший узел мог бы
                # подсветить ячейку, которую блок не читает вовсе.
                ktps_out["hc"] = _floor_idx(a2, t["ktps_tax"])
...
<style>
 ...
 table.map td,table.map th{width:40px;min-width:40px}
 /* редактируемой ячейке нужно на символ больше: «0.76» с полем ввода не влезало
    в 34px и последняя цифра срезалась — числа читались неверно */
 table.map td.ec{width:46px;min-width:46px}
 ...
 body.light td.ec.sel{background:#bcd8f5}
 body.light td.ec.sel input{background:transparent;color:#012}
 ...
 #ctrlbar{display:flex;gap:14px;align-items:center;flex-wrap:wrap;padding:6px 12px;border-bottom:2px solid #2a3340;position:sticky;top:0;z-index:30;background:inherit}
 #ctrlbar select,#ctrlbar input,#ctrlbar button{padding:4px 8px;font-size:13px;border-radius:4px}
 #ctrlbar select{max-width:190px}
 #ctrlbar input[type=checkbox]{padding:0;width:15px;height:15px}
 .cgrp{display:flex;gap:6px;align-items:center}
 .clbl{font-size:12px;color:#8aa;font-weight:600;text-transform:uppercase;letter-spacing:.3px}
 /* СОСТОЯНИЕ ПОРТА — крупный светодиод. Мелкая точка не читалась на ходу и не отличала
    «не подключено» от «подключено, но кадров нет»: оба были серым. Теперь
    красный = связи нет, жёлтый = порт открыт, но кадры не идут, зелёный = идут. */
 .led{width:16px;height:16px;border-radius:50%;display:inline-block;flex:0 0 16px;
      border:1px solid #0006;box-shadow:0 0 0 1px #0003 inset}
 .led.ok{background:#25c04a;box-shadow:0 0 9px #25c04acc}
 .led.warn{background:#f2c11c;box-shadow:0 0 9px #f2c11ccc}
 .led.err{background:#e0392b;box-shadow:0 0 9px #e0392bcc}
 .binbtn{background:#26313d;color:#cde;max-width:280px;overflow:hidden;text-overflow:ellipsis;
         white-space:nowrap;font-weight:600}
 body.light .binbtn{background:#ccd4dd;color:#123}
 #page.locked,#drawer.locked{opacity:.35;pointer-events:none;filter:grayscale(.6)}
 .split{display:flex;gap:12px;padding:10px;align-items:flex-start}
 /* левая колонка ужата на 15%: было 30% ширины окна, стало 25.5%.
    Нижний предел двинут тем же коэффициентом, иначе на узком экране он бы всё вернул. */
 .leftcol{width:25.5%;min-width:221px;max-height:calc(100vh - 70px);overflow:auto;border:1px solid #2a3340;border-radius:8px}
 .rightcol{width:74.5%;flex:1;max-height:calc(100vh - 70px);overflow:auto}
 /* Колонка «Параметр» УЖЕ НА 20%: было ~66% ширины таблицы, стало 53%. Ширину задаём
    жёстко (table-layout:fixed), иначе длинное имя её обратно распирает. Длинные подписи
    переносятся на вторую строку, а не режутся. */
 .logtab{width:100%;border-collapse:collapse;font-size:13px;table-layout:fixed}
 .logtab th:first-child,.logtab td:first-child{width:53%}
 .logtab td:first-child{overflow-wrap:anywhere}
 .logtab th{position:sticky;top:0;background:#1b2129;text-align:left;padding:5px 8px;border-bottom:1px solid #2a3340}
 .logtab td{padding:3px 8px;border-bottom:1px solid #222a33}
 .logtab tr.sticky{background:#182028}.logtab tr.sticky td:first-child{font-weight:700}
 /* числа по ЛЕВОМУ краю — иначе взгляд скачет через всю строку от подписи к значению */
 .logtab .val{text-align:left;font-variant-numeric:tabular-nums;font-weight:600}
 .logtab .unit{color:#8aa;font-size:11px;margin-left:4px}
 .logtab tr[title]{cursor:help}
 .onlinebar{display:flex;gap:8px;align-items:center;margin-bottom:10px;flex-wrap:wrap}
 .onlinebar select,.onlinebar input{padding:4px 8px;font-size:13px}
 /* ---- ящик настроек: всё редкое и опасное убрано сюда ---- */
 #drawer{display:none;background:#131a24;border-bottom:2px solid #2a3340;
         padding:12px 16px;max-height:70vh;overflow:auto}
 #drawer.on{display:block}
 body.light #drawer{background:#eef2f6;border-bottom:2px solid #b3bcc7}
 .dgrp{margin-bottom:16px}
 .dgrp>h3{font-size:12px;margin:0 0 8px;color:#9ab;text-transform:uppercase;letter-spacing:.5px}
 body.light .dgrp>h3{color:#234}
 .plist{display:flex;flex-direction:column;gap:7px;max-width:560px}
 .prow{display:flex;align-items:baseline;gap:10px}
 .prow .pname{width:210px;flex:0 0 210px;font-size:13px}
 .prow input{width:78px;padding:3px 6px;font-size:13px}
 .prow .pdesc{font-size:11px;color:#8aa;line-height:1.35}
 body.light .prow .pdesc{color:#556}
 td.ec.sel{background:#1d3a56}
 td.ec.sel input{background:transparent;color:#dff}
 th.selh{background:#25455f !important;color:#dff !important}
 body.light th.selh{background:#b9d6ef !important;color:#012 !important}
 table.map th.colh,table.map th.rowh{cursor:pointer;user-select:none}
 ...
 .ec{padding:0}
 /* Ячейка САМА и есть поле ввода: ни рамки, ни скругления, ни своего фона. Скруглённый
    инпут внутри клетки рисовал «таблетку» в таблице и ломал сетку. border-radius сбрасываем
    ЯВНО — общее правило select,input выше ставит 6px и на них тоже. */
 .ec input{width:100%;box-sizing:border-box;border:0;border-radius:0;background:transparent;
           text-align:right;font:inherit;color:inherit;padding:2px 4px;outline:none}
 .ec input:focus{background:#0d2a44;box-shadow:inset 0 0 0 2px #6cf}
 body.light .ec input:focus{background:#dceaf8;box-shadow:inset 0 0 0 2px #2a6fb0}
 /* ---- ДАТЧИКИ: крупные цифры на весь экран, для взгляда на ходу ---- */
 #hud{display:none;gap:8px;padding:10px 14px;background:#0e1520;border-bottom:2px solid #2a3a4a;
      flex-wrap:wrap}
 #hud.on{display:flex}
 body.light #hud{background:#e9eef4;border-bottom:2px solid #b3bcc7}
...
<div id=ctrlbar>
 <div class=cgrp><span class=led err id=binicon></span>
   <button class=binbtn id=btnBin onclick=pickBin() title="Выбрать прошивку — откроется обычное окно выбора файла">&#128193; Прошивка не выбрана</button></div>
 <div class=cgrp><span class=clbl>ЭБУ</span><span class="led err" id=ecuicon></span><select id=port onchange=ecuAuto()></select></div>
 <div class=cgrp><span class=clbl>ШДК</span><span class="led err" id=wblicon></span><select id=wport onchange=wblAuto()></select></div>
 <button class=ghost onclick=loadPorts() title="Перечитать список портов">&#8635;</button>
 <span id=portstat style="font-size:12px;color:#8aa"></span>
 <button class=ghost id=btnHud onclick=toggleHud() style="margin-left:auto">&#128200; ДАТЧИКИ</button>
 <button class=ghost id=btnDrawer onclick=toggleDrawer()>&#9881; НАСТРОЙКИ</button>
 <span class=themebox><input type=checkbox id=darkbox onchange=toggletheme()> &#127769;</span>
 <button id=btnEstop class=estop onclick=eStop() title="Сохранить лог и бин, отключить ЭБУ и ШДК, заблокировать страницу">&#9632; СТОП</button>
</div>

<div id=hud class=hud></div>

<div id=drawer>
 <div class=dgrp id=dgrp_nolearn style="display:none">
  <h3>Обучение недоступно</h3>
  <div class=learninfo id=nolearntxt></div>
 </div>
 <div class=dgrp id=dgrp_learn>
  <h3>Обучение по ШДК на ходу</h3>
  <div class=onlinebar>
   ...
   <button class=ghost onclick="learnPreset('fast')" title="Одна поездка, лишь бы прикинуть форму карты. Пускает шум.">грубо, за одну поездку</button>
   <button class=ghost onclick="learnPreset('fine')" title="Доводка почти готовой карты. Шум не пролезает, но набирать точки дольше.">точно, доводка</button>
   <button class=estop id=btnLearnBack onclick=learnRollback() style="display:none">&#8630; ВЕРНУТЬ КАК БЫЛО</button>
  </div>
  <div class=plist id=learnpar>
   <div class=prow><span class=pname>Замеров на ячейку</span><input id=p_min_hits value=3>
     <span class=pdesc>Сколько раз надо попасть в ячейку, прежде чем поправка в неё запишется.</span></div>
   <div class=prow><span class=pname>Доля поправки за раз</span><input id=p_damp value=0.7>
     <span class=pdesc>1.0 — вносить всю разницу сразу. Меньше — вносить частями, спокойнее, но дольше.</span></div>
   <div class=prow><span class=pname>Предел шага, доля</span><input id=p_max_step value=0.2>
     <span class=pdesc>Ограничитель: за один раз ячейка не сдвинется больше чем на эту долю. Страховка от выброса ШДК.</span></div>
   <div class=prow><span class=pname>Не учить ниже газа, %</span><input id=p_tps_min value=0>
     <span class=pdesc>Ниже этого открытия дросселя замеры не берутся. 0 — брать все.</span></div>
   <div class=prow><span class=pname>Задержка выхлопа, мс</span><input id=p_delay_ms value=350>
     <span class=pdesc>Насколько ШДК отстаёт от впрыска: газ дошёл до датчика не сразу. Замер сопоставляется с режимом, который был столько миллисекунд назад.</span></div>
   <div class=prow><span class=pname>Холодно выше (сырьё ТОЖ)</span><input id=p_t_cold value=65>
     <span class=pdesc>Датчик термисторный: чем горячее, тем МЕНЬШЕ число. Выше этого порога мотор считается холодным и обучение стоит.</span></div>
   <div class=prow><span class=pname>Стационар: обороты</span><input id=p_d_rpm value=150>
     <span class=pdesc>Если обороты за окно замера прыгнули сильнее — режим неустановившийся, точка отбрасывается.</span></div>
   <div class=prow><span class=pname>Стационар: газ, %</span><input id=p_d_tps value=3>
     <span class=pdesc>То же по дросселю. На перегазовке смесь заведомо не та, что в карте.</span></div>
   <div class=prow><span class=pname></span><button class=ghost onclick=learnSendPar()>Применить пороги</button></div>
  </div>
 </div>
 <div class=dgrp id=dgrp_learnlog>
  <h3>Обучение по записанному логу (в блок само ничего не пишет)</h3>
  <div class=onlinebar>
   <select id=loglist style="max-width:380px"></select>
   <select id=logwhich><option value=ve>карта VE</option><option value=ktps>карта Ktps</option></select>
   <button class=ghost onclick=learnFromLog()>Прогнать по логу</button>
...
<div class=split>
 ...
 <div class=rightcol>
  <div id=connwarn class=divbar></div>
  ...
  <div class=onlinebar>
   <select id=mapsel title="какую карту сделать живой — тень одна, живёт ровно одна карта">
    ...
    <option value=ve class=dadonly>карта: VE (ДАД)</option>
    <option value=ktps class=dadonly>карта: Ktps (ДАД)</option>
   </select>
  ...
  <!-- ВСЕ карты живут ВНУТРИ этого блока. Живая карта поднимается наверх ТОЛЬКО здесь:
       раньше её клали в начало всей колонки, и она уезжала выше кнопок и плашек. -->
  <div id=mapsarea>
...
function setStat(id,st){const e=document.getElementById(id);if(e)e.className='led '+st;}
// Список портов перестраивается ПОЛНОСТЬЮ, но выбранный порт не теряется, даже если
// система его больше не показывает (уснул адаптер, отвалился мост): он остаётся в списке
// с пометкой. Иначе select молча обнулялся, и выглядело это как «кнопка не работает».
async function loadPorts(){
 const note=document.getElementById('portstat');
 try{const r=await fetch('/api/ports');const d=await r.json();
  for(const sid of ['port','wport']){
    ...
    const ph=document.createElement('option');ph.value='';ph.textContent='— не подключено —';s.appendChild(ph);
    let seen=false;
    d.ports.forEach(p=>{if(busy&&p.device===busy)return;
      if(p.device===cur)seen=true;
      const o=document.createElement('option');o.value=p.device;
      o.textContent=p.device+(p.ftdi?' (FTDI)':'')+(p.desc&&!p.ftdi?' — '+p.desc:'');s.appendChild(o);});
    if(cur&&!seen&&cur!==busy){const o=document.createElement('option');o.value=cur;
      o.textContent=cur+' — нет в системе';s.appendChild(o);}
    if(cur&&cur!==busy)s.value=cur;
    else if(sid==='port'&&d.suggested&&d.suggested!==busy&&!s.value)s.value=d.suggested;
  }
  if(note){note.textContent='портов: '+d.ports.length;
    setTimeout(()=>{if(note.textContent.startsWith('портов'))note.textContent='';},2500);}
 }catch(e){if(note)note.textContent='список портов не прочитался';}
}
// ---- ПРОШИВКА: родное окно выбора файла, а не выпадашка по двум папкам ----
let binPath='';
function shortBin(name){
 if(!name)return 'Прошивка не выбрана';
 let s=name.replace(/\.bin$/i,'').replace(/\s*ИИ$/,'').replace(/^j30_/i,'');
 s=s.replace(/_\d\d\.\d\d\.\d\d(_\d{4})?\s*$/,'');   // хвост «_10.08.26_0055» не несёт смысла в кнопке
 s=s.replace(/_/g,' ').trim();
 return s.length>34?s.slice(0,33)+'…':s;
}
function setBinBtn(name,path){
 binPath=path||'';
 const b=document.getElementById('btnBin');
 b.textContent='📁 '+shortBin(name);
 b.title=path?('Выбрана прошивка:\n'+path+'\n\nНажми, чтобы выбрать другую'):'Нажми, чтобы выбрать прошивку';
 binLock();
}
async function loadBins(){   // при загрузке страницы — показать, что уже выбрано на сервере
 try{const r=await fetch('/api/bins');const d=await r.json();
  const p=d.selected||'';setBinBtn(p?p.split('/').pop():'',p);}catch(e){}
}
function binLock(){const has=!!binPath;
 document.getElementById('page').classList.toggle('locked',!has);
 // ящик настроек лежит ВНЕ #page — блокируем его отдельно, иначе обучение
 // запускалось бы без выбранной прошивки, то есть без карт, по которым учить
 document.getElementById('drawer').classList.toggle('locked',!has);
 setStat('binicon',has?'ok':'err');}
async function pickBin(){
 const b=document.getElementById('btnBin');const old=b.textContent;
 b.textContent='📁 окно выбора…';b.disabled=true;
 try{
  const r=await fetch('/api/pickbin',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
...
// ВСЕГДА два знака после запятой. Разнобой (14.7 / 0.968 / 25) не давал сравнивать
// соседние ячейки взглядом — колонки прыгали по ширине. Вводить можно и три знака,
// показывается всё равно два.
function fmt2(v){return (v===null||v===undefined||isNaN(v))?'—':Number(v).toFixed(2);}
function axhead(cols){let h='';for(let c=0;c<16;c++)h+='<th class=colh data-c='+c+'>'+cols[c]+'</th>';return h;}
// У карт РАЗНЫЕ горизонтальные оси. Подпись «об\нагр» на карте VE врала: там давление,
// а у Ktps — открытие дросселя.
const AXNAME={fuel:'об\\нагр',ign:'об\\нагр',ve:'об\\кПа',ktps:'об\\газ%'};
function renderMap(m,prefix){
 let h='<table class=map><tr><th class=corner>'+(AXNAME[prefix]||'об\\нагр')+'</th>'+axhead(m.cols)+'</tr>';
 for(let r=0;r<16;r++){h+='<tr><th class=rowh data-r='+r+'>'+m.rows[r]+'</th>';
  for(let c=0;c<16;c++){const v=m.cells[r*16+c];
   const cls=(v===null)?' class=flag':'';const disp=(v===null)?'фл':fmt2(v);
...
// ---- левая таблица логируемых (стики в топе, значение + короткая единица) ----
// ПОРЯДОК ВЕРХНЕЙ СВОДКИ задан пользователем: сначала то, по чему настраивают
// (обороты — нагрузка — VE — цель — факт), потом всё прочее.
// «Поправка VE (факт/цель)» из сводки убрана: величина производная, без ШДК пустая
// и посмотреть по ней нечего. Вместо неё — ТЕКУЩЕЕ VE, которое блок реально применил.
// Пятый столбец = «только для ДАД». На расходомерной прошивке такие строки не
// показываем вовсе: величины там не считаются, и в ячейках лежит мусор, а не ноль.
const TOP=[['Обороты','rpm','','16 бит $140A × 12.807',0],
           ['Нагрузка TP','tp','','$1482 / 8 — по нему берётся столбец карт',0],
           ['VE текущее','ve','×','$00FA — коэффициент наполнения, который блок применил сейчас',1],
           ['AFR цель','afr_target','','из карты смеси по текущей ячейке',0],
           ['AFR факт','afr_fact','','широкополосник; пусто — данных нет',0],
           ['Ktps','ktps','×','$00FB — поправка по дросселю',1],
           ['Давление','press','кПа','$00F7 — абсолютное во впуске',1],
           ['Газ','tps_pct','%','от выученного нуля дросселя',0],
           ['УОЗ','uoz_deg','°','70 − $140F, реально доставленный',0],
           ['Впрыск','inj_ms','мс','$004D × 0.010',0],
           ['Загрузка форсунок','inj_duty','%','мс × об / 1200',0],
           ['Темп ОЖ','temp','','сырьё АЦП: меньше — горячее',0],
           ['ALPHA','alpha','×','коррекция по лямбде',0]];
let isDad=false;
function topList(){return TOP.filter(t=>!t[4]||isDad);}
let varsBuilt='';
function renderVars(vars,top){
 const tb=document.getElementById('vars');
 const L=topList(), key=(isDad?'d':'m')+'/'+vars.length;
 if(varsBuilt!==key){
  let h='';
  L.forEach((t,j)=>{h+='<tr class=sticky title="'+t[3]+'"><td>'+t[0]+'</td><td class=val id=tv'+j+'></td></tr>';});
  for(let i=0;i<vars.length;i++){
   const hint=(vars[i].hint||'').replace(/"/g,'&quot;');
   h+='<tr'+(hint?' title="'+hint+'"':'')+'><td>'+vars[i].name+'</td><td class=val id=vv'+i+'></td></tr>';}
  tb.innerHTML=h;varsBuilt=key;}
 L.forEach((t,j)=>{const c=document.getElementById('tv'+j);if(!c)return;
  const raw=(top&&top[t[1]]!=null)?top[t[1]]:null;
  const v=(raw===null)?'—':(raw+(t[2]?(' '+t[2]):''));
  if(c.textContent!==v)c.textContent=v;});
 for(let i=0;i<vars.length;i++){const v=vars[i];const c=document.getElementById('vv'+i);
  // Единица — ОДНО короткое слово. Всё пояснение висит в тултипе строки, а не в тексте:
  // приписки вида «сырьё (/8=TP)» занимали полколонки и ничего не объясняли.
  let disp;
  if(v.val===null)disp='—';
  else if(v.real!==null)disp=v.real+' <span class=unit>'+(v.unit||'у.е.')+'</span>';
  else disp=v.val+' <span class=unit>'+(v.unit||'у.е.')+'</span>';
...
let dadWas=null, dadOffWas=null;
function setDadMode(on,off){
 if(on===dadWas&&off===dadOffWas)return;      // DOM трогаем только на смене прошивки
 dadWas=on; dadOffWas=off; isDad=on;
 for(const id of ['dgrp_learn','dgrp_learnlog']){
  const e=document.getElementById(id); if(e)e.style.display=on?'':'none';}
 const no=document.getElementById('dgrp_nolearn'); if(no)no.style.display=on?'none':'';
 const nt=document.getElementById('nolearntxt');
 if(nt)nt.innerHTML = off
  ? 'Прошивка <b>умеет в ДАД, но переключена в режим расходомера</b> (скаляр «РЕЖИМ РАСЧЁТА» = 0). '+
    'Рутина ДАД уходит в заводскую таблицу линеаризации первой же командой, до карт VE и Ktps дело не доходит — '+
    'поэтому они скрыты и обучение выключено. Хочешь настраивать VE — переключи режим в 1 в редакторе и перешей.'
  : 'Выбрана прошивка на <b>расходомере</b>. Обучение правит карты VE и Ktps, а их в такой прошивке нет: '+
    'форма наполнения зашита в заводскую таблицу линеаризации расходомера. '+
    'Смесь и угол настраиваются вручную в таблицах на странице.';
 const sel=document.getElementById('mapsel');
 if(sel){
  Array.from(sel.querySelectorAll('option.dadonly')).forEach(o=>{o.hidden=!on;o.disabled=!on;});
  // выбранной осталась карта, которой в этой прошивке нет — вернуть на смесь,
  // иначе ОНЛАЙН уйдёт за таблицей, которой в бине не существует
  if(!on&&(sel.value==='ve'||sel.value==='ktps'))sel.value='fuel';
 }
 if(!on&&learnOn){   // прошивку сменили на ходу — обучение молча оставлять нельзя
  learnOn=false;
  fetch('/api/learn/stop',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'}).catch(_=>{});
  const b=document.getElementById('btnLearn'); if(b)b.classList.remove('on');
 }
}
...
function raiseLive(){
 // ВНУТРИ #mapsarea, а не в колонке целиком: раньше карта уезжала выше кнопок,
 // плашек и строки состояния — управление оказывалось под таблицей.
 const par=document.getElementById('mapsarea'); if(!par)return;
...
function renderEdit(which){
 ...
 let h='<table class="map edit" data-w='+which+'><tr><th class=corner>'+(AXNAME[which]||'об\\нагр')+'</th>'+axhead(ax.cols)+'</tr>';
 for(let r=0;r<16;r++){h+='<tr><th class=rowh data-r='+r+'>'+ax.rows[r]+'</th>';
  for(let c=0;c<16;c++){const idx=r*16+c,b=arr[idx];
   h+='<td id='+which+'_'+r+'_'+c+' class=ec data-r='+r+' data-c='+c+' title="байт '+b+'">'+
     '<input value="'+fmt2(physOf(which,b))+'" onchange="cellEdit(\''+which+'\','+idx+',this)"></td>';}
  h+='</tr>';}
...
async function cellEdit(which,idx,el){
 ...
 if(which!==liveMap){el.value=fmt2(physOf(which,ONLINE[which][idx]));
 ...
 if(isNaN(val)){el.value=fmt2(physOf(which,ONLINE[which][idx]));return;}
 ...
 if(d.ok&&d.applied){ONLINE[which][idx]=byte;saveProg();td.classList.add('ok');td.title='байт '+byte;el.value=fmt2(physOf(which,byte));}
...
// ---- ВЫДЕЛЕНИЕ ОБЛАСТЬЮ + ПО ОСЯМ + МАССОВАЯ ПРАВКА ----
// Было: выделялось всё, по чему протащил мышь — «след кисти». Промахнулся строкой —
// в выделении оказались чужие ячейки, и заметить это на 256 клетках невозможно.
// Стало: как в таблицах на рабочем столе — зажал ЛКМ и тянешь ПРЯМОУГОЛЬНИК.
// Клик по заголовку столбца/строки берёт его целиком, протяжка по заголовкам — диапазон.
let selCells=new Set(), selMode=null, anchorRC=null, dragging=false;
function clearSel(){
 selCells.forEach(td=>td.classList.remove('sel'));selCells.clear();
 document.querySelectorAll('table.map th.selh').forEach(t=>t.classList.remove('selh'));
 const b=document.getElementById('bulk'); if(b)b.style.display='none';}
function editTable(){return document.querySelector('table.map.edit');}
function markSel(r0,r1,c0,c1){
 const t=editTable(); if(!t)return;
 selCells.forEach(td=>td.classList.remove('sel'));selCells.clear();
 t.querySelectorAll('th.selh').forEach(x=>x.classList.remove('selh'));
 const rA=Math.min(r0,r1),rB=Math.max(r0,r1),cA=Math.min(c0,c1),cB=Math.max(c0,c1);
 t.querySelectorAll('td.ec').forEach(td=>{
  const r=+td.dataset.r,c=+td.dataset.c;
  if(r>=rA&&r<=rB&&c>=cA&&c<=cB){td.classList.add('sel');selCells.add(td);}});
 // подсветка самих заголовков — видно, какой диапазон осей захвачен
 t.querySelectorAll('th.rowh').forEach(th=>{const r=+th.dataset.r;
  if(r>=rA&&r<=rB&&cA===0&&cB===15)th.classList.add('selh');});
 t.querySelectorAll('th.colh').forEach(th=>{const c=+th.dataset.c;
  if(c>=cA&&c<=cB&&rA===0&&rB===15)th.classList.add('selh');});
}
function showBulk(){const b=document.getElementById('bulk');
 if(selCells.size>0){document.getElementById('bulkn').textContent=selCells.size;b.style.display='flex';
  const inp=document.getElementById('bulkv');
  if(inp)inp.placeholder=(liveMap==='fuel')?'значение AFR':(liveMap==='ign')?'градусы':'коэффициент';}
 else{b.style.display='none';}}
function rcUnder(e){
 const el=document.elementFromPoint(e.clientX,e.clientY); if(!el)return null;
 const td=el.closest('td.ec'); if(td)return {r:+td.dataset.r,c:+td.dataset.c};
 const rh=el.closest('th.rowh'); if(rh)return {r:+rh.dataset.r,c:null};
 const ch=el.closest('th.colh'); if(ch)return {r:null,c:+ch.dataset.c};
 return null;}
document.addEventListener('mousedown',e=>{
 if(e.button!==0)return;
 const t=e.target;
 const rh=t.closest&&t.closest('th.rowh'), ch=t.closest&&t.closest('th.colh'), td=t.closest&&t.closest('td.ec');
 if(rh&&rh.closest('table.map.edit')){e.preventDefault();selMode='rows';anchorRC={r:+rh.dataset.r,c:null};
  dragging=true;document.body.classList.add('nosel');markSel(anchorRC.r,anchorRC.r,0,15);showBulk();return;}
 if(ch&&ch.closest('table.map.edit')){e.preventDefault();selMode='cols';anchorRC={r:null,c:+ch.dataset.c};
  dragging=true;document.body.classList.add('nosel');markSel(0,15,anchorRC.c,anchorRC.c);showBulk();return;}
 if(td){selMode='cells';anchorRC={r:+td.dataset.r,c:+td.dataset.c};dragging=false;clearSel();return;}
 // клик мимо таблицы — снять выделение, чтобы «применить ко всем» не сработало по забытому
 if(!t.closest||!t.closest('#bulk'))clearSel();
 selMode=null;anchorRC=null;});
document.addEventListener('mousemove',e=>{
 if(!selMode||!anchorRC||e.buttons!==1)return;
 const p=rcUnder(e); if(!p)return;
 if(selMode==='cells'){
  if(p.r===null||p.c===null)return;
  if(!dragging){dragging=true;document.body.classList.add('nosel');
   const t=editTable(); if(t){const inp=t.querySelector('input:focus');if(inp)inp.blur();}}
  markSel(anchorRC.r,p.r,anchorRC.c,p.c);
 }else if(selMode==='rows'){
  if(p.r===null)return; markSel(anchorRC.r,p.r,0,15);
 }else if(selMode==='cols'){
  if(p.c===null)return; markSel(0,15,anchorRC.c,p.c);
 }
 showBulk();});
document.addEventListener('mouseup',()=>{
 if(dragging)showBulk();
 dragging=false;selMode=null;anchorRC=null;document.body.classList.remove('nosel');});
...
function selectAllCells(){
 // выделить всю живую карту: возить мышью по 256 ячейкам незачем
 if(!liveMap){document.getElementById('onstat').textContent='живой карты нет';return;}
 markSel(0,15,0,15);showBulk();}
function wireBulk(){
...
function syncPtr(p,conn){
 ...
 const on=!!p.online; const NM={fuel:'СМЕСЬ',ign:'УГОЛ',ve:'VE',ktps:'Ktps'};
 // тень ОДНА на 256 байт: живой может быть ровно одна карта, остальные читают ПЗУ
...
async function eStop(){
 ...
 // «СТОП...» с многоточием читалось как непонятный мусор. Пишем словами, что идёт.
 b.disabled=true; b.textContent='СОХРАНЯЮ И ОТКЛЮЧАЮСЬ';
 ...
 setBinBtn('','');                // нет прошивки → #page.locked → все действия выключены
 document.getElementById('port').value='';
 document.getElementById('wport').value='';
 setDiverged(false);
 let msg='ОСТАНОВЛЕНО. ';
 msg += d.bin ? ('прошивка сохранена: '+d.bin+'. ')
              : (d.same ? 'прошивка не сохранена — карты не менялись. ' : 'прошивка не сохранена — карт на руках не было. ');
 msg += d.log ? ('лог сохранён: '+d.log+'. ') : 'лог не сохранён — данных не было. ';
 ...
 b.innerHTML='&#9632; СТОП';   // disabled вернёт tick по фактическому состоянию
}
// ---- тик ----
function tick(){fetch('/api/status').then(r=>r.json()).then(d=>{
 ...
 // СВЕТОФОР: КРАСНЫЙ = не подключено (порт не выбран, не открылся или ошибка),
 // ЖЁЛТЫЙ = порт открыт, но кадры не идут, ЗЕЛЁНЫЙ = кадры идут.
 setStat('ecuicon', (d.error||!portSel||!d.running)?'err':((d.rate>0||d.busy)?'ok':'warn'));
 const wb=d.wbl||{};
 // зелёный ТОЛЬКО пока данные свежие: раньше хватало непочищенного raw, и значок
 // оставался зелёным после выдернутого провода
 setStat('wblicon', (wb.error||!wportSel||!wb.running)?'err':(wb.fresh?'ok':'warn'));
 // ОШИБКУ СВЯЗИ НАДО ВИДЕТЬ. Раньше она жила только внутри сервера, а на экране
 // был просто значок другого цвета — понять, что случилось, было нельзя.
 const cw=document.getElementById('connwarn');
 const emsg=d.error||(wb.error?('Широкополосник: '+wb.error):'');
 if(cw){cw.textContent=emsg?('⚠ '+emsg):'';cw.style.display=emsg?'':'none';}
 ...
 // РЕЖИМ ПРОШИВКИ. На расходомерной убираем всё, чего в ней физически нет:
 // карты VE/Ktps, их строки в сводке и обучение. Иначе панель предлагает настроить
 // то, чего блок не читает, и показывает мусор из неинициализированных ячеек.
 setDadMode(!!(d.top&&d.top.is_dad), !!(d.top&&d.top.dad_off));
 renderVars(d.vars||[], d.top||{});
 hudShow(d.top||{});
 // СТОП доступен, только когда есть что останавливать. Раньше он нажимался всегда,
 // в том числе при отключённом блоке, и «останавливал» ничего.
 const be=document.getElementById('btnEstop');
 if(be&&be.textContent.indexOf('СОХРАНЯЮ')<0){
  const busy=d.running||(d.wbl&&d.wbl.running)||(d.log&&d.log.on)||online;
  be.disabled=!busy;
  be.title=busy?'Сохранить лог и прошивку, отключить ЭБУ и ШДК, заблокировать страницу'
               :'Останавливать нечего: ЭБУ и ШДК не подключены, лог не пишется';
 }
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.send_header("Content-Length", str(len(body))); self.end_headers(); self.wfile.write(body)
    def do_POST(self):
        # Любая необработанная ошибка (в первую очередь — оборванный порт посреди заливки)
        # раньше валила ответ целиком: страница получала пустоту и молчала. Теперь ошибка
        # доезжает до пользователя текстом.
        try:
            self._do_post()
        except Exception as e:
            try: self._json({"ok": False, "error": "Операция не выполнена: %s" % e}, 200)
            except Exception: pass
    ...
    def _do_post(self):
        ...
        if self.path.startswith("/api/selectbin"):
            p = data.get("bin", "")
            if p == "":
                SEL["bin"] = ""                    # сброс в «не выбрано»
                self._json({"ok": True}); return
            # ⚠ РАНЬШЕ несуществующий путь МОЛЧА оставлял прежнюю прошивку и отвечал «ок».
            # Панель показывала карты и обучение от СТАРОГО файла, будучи уверенной, что
            # открыт новый. На таком и правки уходят не в ту таблицу.
            if not os.path.exists(p):
                self._json({"ok": False, "error": "Файл прошивки не найден: %s" % os.path.basename(p)}); return
            if os.path.getsize(p) != 32768:
                self._json({"ok": False, "error": "Это не прошивка ЭБУ: размер %d байт, нужно 32768" % os.path.getsize(p)}); return
            SEL["bin"] = p
            _tab_cache.pop(p, None)
            self._json({"ok": True}); return
        if self.path.startswith("/api/pickbin"):
            # родное окно выбора файла — любая папка, не только логер/ и j30/
            start = os.path.dirname(SEL["bin"]) if SEL["bin"] else HERE
            p, err = pick_bin_dialog(start)
            if p:
                SEL["bin"] = p
                _tab_cache.pop(p, None)            # выбрали заново → перечитать с диска
            self._json({"ok": bool(p), "path": p, "name": os.path.basename(p) if p else "",
                        "error": err}); return
        ...
        if self.path.startswith("/api/estop"):
            ...
            res = {"bin": "", "log": "", "errors": [], "same": False}
            name, err = _bake_bin(data, suffix="_СТОП_")
            res["bin"] = name
            if err == "без изменений": res["same"] = True
            elif err and err != "бин не выбран": res["errors"].append("бин: " + err)
            for fn_, tag in ((log_stop, "лог"), (do_stop, "ЭБУ"), (wbl_stop, "ШДК")):
                try: fn_()
                except Exception as e: res["errors"].append("%s: %s" % (tag, e))
            # ИМЯ ЛОГА БЕРЁМ ПОСЛЕ ОСТАНОВКИ: log_stop чистит путь, если не записалось
            # ни одной строки, — тогда файла нет и хвалиться нечем.
            try: res["log"] = os.path.basename(LOGST["path"]) if LOGST["path"] else ""
            except Exception: pass
