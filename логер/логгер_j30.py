...
LABELS = [
    ...
    (0x14DE, "Обогащение ускорения",  8, "у.е.",               None),   # >0 = было обогащение
    (0x141A, "Смесь из карты (сырьё)",8, "у.е.",               None),
    ...
    (0x1492, "АЦП Дроссель", 16, "у.е.",              None),   # аналог. дроссель, ch6
...
HINTS = {
    ...
    0x14DE: "Добавка топлива при открытии дросселя. Больше нуля — обогащение работало.",
    0x141A: "Что вернула выборка из карты смеси, до коррекций.",
    ...
    0x1492: "Сырьё АЦП дросселя, 10 бит.",
...
def load_tables(path):
    ...
    dad = ((b[0x0965] == 0xBD and b[0x0966] == 0xC7 and b[0x0967] == 0x00)
...
def _keepalive_loop(ser, stop_ev):
    """Раз в секунду шлём мосту служебный байт: «панель на этом канале, я живая».
...
<div id=drawer>
 <div class=dgrp id=dgrp_nolearn style="display:none">
  ...
  <div class=learninfo>Выбрана прошивка на <b>расходомере</b>. Обучение правит карты VE и Ktps,
   а их в такой прошивке нет: форма наполнения зашита в заводскую таблицу линеаризации расходомера.
   Смесь и угол настраиваются вручную в таблицах на странице.</div>
 </div>
...
let dadWas=null;
function setDadMode(on){
 if(on===dadWas)return;                       // DOM трогаем только на смене прошивки
 dadWas=on; isDad=on;
