...
class Machine:
    ...
    def start_engine(self, boot_steps=80_000):
        ...
        # включено»). Но boot двойника не шлёт прерываний, поэтому чистка срабатывает
    ...
    P_RELAX = 12.0          # инерция маховика (плавность; демпфирует круиз под нагрузкой)
    P_LOAD_K = 100.0        # коэф. дорожной нагрузки (тормозной момент ∝ оборотам² против мотора)
    P_THR_CAP = 60000.0     # пропускная способность дросселя на полном открытии (приток)
    P_PUMP_FILL = 20.0      # прокачка мотора-насоса на оборот (полное наполнение коллектора)
    ...
    # КРИВАЯ НАПОЛНЕНИЯ ЦИЛИНДРА (воздух на оборот) по оборотам. На НИЗАХ выше —
    # при открытом газе цилиндр успевает набиться полнее (выше объёмный КПД); к
    # верхам спадает (прокачка упирается, маф клампится). Постоянное наполнение
    # занижало нагрузку на низах -> «низкие об + высокая нагрузка» (тяга внатяг) не
    # получалось, и клапаны (EGR, мощности) не попадали в своё окно по нагрузке.
    P_FILL_RPM = [600, 800, 1200, 1600, 2000, 2750, 4000, 6000, 8000]
    P_FILL_K   = [42,  40,  30,   24,   21,   20,   15,   10,   8]

    def _pump_fill(self, rpm):
        """Наполнение цилиндра (воздух на оборот) — линейная интерполяция по оборотам."""
        xs, ys = self.P_FILL_RPM, self.P_FILL_K
        if rpm <= xs[0]:
            return ys[0]
        if rpm >= xs[-1]:
            return ys[-1]
    ...
    def engine_plant_step(self, throttle, coolant=20, airtemp=20, base_air=None,
                          ignition=True, starter=False, ticks=12, road_load=0.0):
        ...
        # TPS — НАСТОЯЩИЙ АНАЛОГ через АЦП канал 3 (потенциометр 0.5..~4.5В), как в
        # железе: блок сам читает канал 3 в $1400 (=сырьё×256). Закрыт=0.5В->сырьё 26,
        # полный газ=~4.5В->сырьё 230.
        tps_raw = int(26 + thr * 204)
        self.bus.adc[3] = (tps_raw & 0xFF) << 8
        # ($14A2-позицию блок считает сам, но в двойнике она застряла на 44 — мой инжект
        #  он затирает, так что не трогаем: WOT-обогащение и так идёт через карту топлива.)
        # КОНЦЕВИК ХХ -> $15.4: дроссель закрыт = холостой. Блок в двойнике из аналога
        # его не выводит, поэтому инжектим (бит проверен: блок тестит TIM #$10,$15).
        if thr <= 0.0:
        ...
        # --- ВОЗДУХ: ДРОССЕЛЬ-СУЖЕНИЕ + НАПОЛНЕНИЕ КОЛЛЕКТОРА (speed-density) ---
        # Приток через щели (обвод винт+РХХ; дроссель ∝ открытию) ОГРАНИЧЕН тем,
        # сколько мотор-насос успевает откачать (∝ оборотам при полном коллекторе):
        #     воздух = min(приток, прокачка)
        #   • частичный газ на оборотах -> душит ДРОССЕЛЬ (приток < прокачки), мало воздуха
        #   • низкие обороты + открытый газ (ПОД НАГРУЗКОЙ) -> душит МОТОР: коллектор
        #     набивается полнее -> воздух/цикл РАСТЁТ -> нагрузка $1413 лезет в окно EGR
        #   • WOT -> душит мотор (полное наполнение), воздух максимальный
        duty = self._eff_duty
        inflow = base_air + self.P_KAIR * duty + self.P_THR_CAP * thr     # приток (щели открыты)
        pump = self._pump_fill(self.plant_rpm) * self.plant_rpm           # прокачка: наполнение(об) × об
        air = min(inflow, pump)
        ...
        if self.cranking:
            ...
            # ДОРОЖНАЯ НАГРУЗКА (Ур.1): тормозной момент против мотора ∝ ОБОРОТАМ²
            # (аэродинамика ∝ скорости²). Квадрат важен для УСТОЙЧИВОСТИ: на высоких
            # оборотах нагрузка обгоняет момент и держит мотор на КРУИЗЕ, а не даёт
            # улететь. Под нагрузкой, чтоб держать обороты, нужно больше газа -> больше
            # воздуха -> наполнение коллектора -> нагрузка $1413 лезет в окно (EGR и пр.).
            # road_load: 0 = нейтраль (как было), 1 = тяжело (в гору/груз).
            self.plant_rpm -= self.P_LOAD_K * road_load * (self.plant_rpm / 1000.0) ** 2
        if self.plant_rpm < 0:
            self.plant_rpm = 0.0
        ...
        st["road_load"] = round(road_load * 100)  # дорожная нагрузка, %
