...
DEFS = [
 ...
 {"a":0x4A13,"len":1,"kind":"scalar","conv":None,"name":"РЕЖИМ РАСЧЁТА: 0 = расходомер, 1 = ДАД","own":True,"note":"Одна прошивка на оба случая. 0 — наполнение считает ЗАВОДСКОЙ код по расходомеру ($8057), как в стоке; всё, что ниже про ДАД (VE, Ktps, Смещ, Наклон, КМ), не участвует. Любое ненулевое — работает speed-density по датчику давления. По умолчанию 0: без подключённого ДАД режим 1 считал бы мусор. Проверяется в самом начале рутины C700."},
