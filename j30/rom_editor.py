...
CONV = {
    ...
    # Смещение ЗНАКОВОЕ: −128…+127 отсчётов, то есть примерно −2.5…+2.5 В.
    # Минус — нормальная характеристика широкодиапазонного датчика (показывает
    # давление уже при нуле). Раньше байт был беззнаковым, и такие датчики
    # не описывались вовсе: калькулятор ругался «вне диапазона».
    "dadofs":  ("В",      lambda x: round((x - 256 if x > 127 else x) * 0.0195, 2),
                          lambda v: max(-128, min(127, round(float(v) / 0.0195))) & 0xFF),
...
DEFS = [
 ...
 {"a":0x4900,"len":256,"kind":"map16","conv":"ve","name":"VE — карта наполнения (ДАД speed-density)","rax":0x4A20,"cax":0x4A00,"own":True,"note":"Коэффициент наполнения (0…2, 128=1.0) по [обороты × давление]. ОСНОВНАЯ карта тюна speed-density: наполнение = Д·VE·Ktps·КМ. Крутишь по ШДК: бедно/богато в точке → правишь ячейку. ОБЕ оси теперь СВОИ и равномерные (0x4A20 обороты, 0x4A00 давление). Рутина @ C700 ищет ячейку САМА, сдвигами, и интерполирует по давлению; заводской lookup 80CF больше не используется — он брал ось из $1482 и требовал SEI."},
 {"a":0x4A00,"len":16,"kind":"row","conv":"kpa","name":"Ось ДАВЛЕНИЯ для VE, кПа (ДАД)","own":True,"note":"16 точек оси давления карты VE, в кПа. ⚠ ОСЬ РАВНОМЕРНАЯ, шаг 8 от 16 (16…136). Так сделано не для красоты: рутина берёт номер столбца СДВИГОМ, без перебора по оси и без деления — только поэтому весь расчёт помещается в один такт и не нужен SEI. Менять точки по одной НЕЛЬЗЯ, код этого не увидит. Под наддув шаг меняется в билдере (PSTEP=16 накроет 16…256 кПа) вместе с числом сдвигов в блоке «столбец по давлению»."},
 {"a":0x4A20,"len":16,"kind":"row","conv":"rpm50","name":"Ось ОБОРОТОВ для VE и Ktps (ДАД)","own":True,"note":"Своя ось оборотов наших карт, отдельная от заводских, но по виду такая же: 400…6400 равномерно. ⚠ Шаг обязан быть степенью двойки — номер строки берётся сдвигом, без перебора по оси. По оборотам рутина ИНТЕРПОЛИРУЕТ, как заводской 80CF: последняя точка работает правым концом интервала, а всё ниже первой точки прижимается к ней (8138-8144). Поэтому карта не прыгает ступенькой между строками. Раньше карты VE и Ktps сидели на заводской оси 0x7B20, но она неравномерная (…32,36,40…), и сдвигом по ней не попасть. Правится только целиком, в билдере (RBASE/RSTEP)."},
 {"a":0x4A10,"len":1,"kind":"scalar","conv":"dadofs","name":"Смещение ДАД, В (тарировка)","own":True,"note":"Напряжение датчика при 0 кПа, в ВОЛЬТАХ. ЗНАКОВОЕ: −2.5…+2.5 В. Для 0.5-4.5В/0-100кПа = 0.5В. МИНУС — НОРМА для широкодиапазонных: они показывают давление уже при нуле вольт, и продлённая назад прямая пересекает 0 кПа в отрицательных вольтах (тойотовский 89421-30100: −1.18 В). Раньше байт был беззнаковым и такие датчики не описывались вовсе. Вместе с Наклоном: кПа = (В_датчика − Смещение)×Наклон. Вводишь в вольтах — редактор сам в отсчёты АЦП для кода."},
 ...
 {"a":0x4A13,"len":1,"kind":"scalar","conv":None,"name":"РЕЖИМ РАСЧЁТА: 0 = расходомер, 1 = ДАД","own":True,"note":"Одна прошивка на оба случая. 0 — рутина сразу уходит в заводскую таблицу линеаризации расходомера ($802C) хвостовым переходом, и путь становится НЕОТЛИЧИМ от стокового (сверено на двойнике побайтно: наполнение, вся zero-page, всё ОЗУ переменных); всё, что ниже про ДАД (VE, Ktps, Смещ, Наклон, КМ), не участвует. Любое ненулевое — speed-density по датчику давления. По умолчанию 0: без подключённого ДАД режим 1 считал бы мусор. Проверяется первой же командой рутины C700."},
 {"a":0x4B00,"len":256,"kind":"map16","conv":"ve","name":"Ktps — ПОПРАВКА ПО ДРОССЕЛЮ (домножается на VE)","rax":0x4A20,"cax":0x4C00,"own":True,"note":"Доп. коэффициент по [обороты × % дросселя]. Наполнение = Д·VE·Ktps·КМ — VE и Ktps ПЕРЕМНОЖАЮТСЯ (как в январе поправки по давлению и дросселю). 1.0=без поправки (номинал 0x80). Крутишь если смесь зависит от ПОЛОЖЕНИЯ дросселя при том же давлении/оборотах. Индекс — открытие $14A2 (от выученного нуля, 0=закрыт). Ось оборотов общая с VE (0x4A20). Ось дросселя осталась январской, неравномерной: по ней рутина ищет перебором, как заводской 80CF, и без интерполяции — поправка к такой тонкости не чувствительна. С 09.08.26 считается на том же такте, что и давление, а не раз в проход основного цикла: на перегазовке больше не опаздывает."},
 ...
 {"a":0x7E20,"len":16,"kind":"row","conv":"msraw","name":"ПУСКОВОЕ обогащение vs темп (Crank Enrich)","note":"холод=96 → тепло=6. Показ в «мс» — ИНТЕРПРЕТАЦИЯ M30, из кода J30 масштаб тик→мс НЕ выведен. ⚠⚠ КРИТИЧНО ПРИ СМЕНЕ ФОРСУНОК: значение из этой таблицы × ($142E+100) идёт в длительность впрыска НАПРЯМУЮ и берётся ПО МАКСИМУМУ с основным расчётом (код 8a92-8ad8, максимум на 8ad0 CPX/STX $1411). Мимо и K, и множителя 0x0A81. Поставишь форсунки больше и не тронешь таблицу — те же единицы зальют больше топлива → ЗАХЛЁБЫВАНИЕ и залив свечей на холодном пуске (там значения максимальные). Масштабировать: значение × (220 / поток_новых). ⚠ У таблицы ДВА ЖИВЫХ ЧИТАТЕЛЯ, оба масштабируются вместе с ней: (1) 8a94 — обогащение, идёт в максимум с основным расчётом; (2) b13a-b145 — выборка × 125 кладётся ПРЯМО в $004D, минуя и $1411, и $144E, то есть это отдельный пусковой импульс в железо. Третье обращение c3ca — в МЁРТВОЙ ветке $C300-$C5FF, недостижимо. ⚠ ПОСЛЕ масштабирования проверить пуск на моторе: базовый пусковой расчёт (8b26-8b51) множителем НЕ масштабируется, и если опустить эту таблицу ниже него — упрёшься в него как в пол. Замер: в стоке таблица даёт 896-1024 против базовых 350-460, запас есть. (Прежнее примечание ссылалось на c3c8-c3d7 как на рабочий код — ошибка, ветка мёртвая; исправлено 10.08.26.)"},
 ...
 {"a":0x0A81,"len":2,"kind":"scalar16","conv":"vq16","name":"ФОРСУНКИ — множитель длительности впрыска, 16-бит","note":"сток=33280 (0x8200). ⚠ ЭТО ГЛАВНАЯ РУЧКА ПРОИЗВОДИТЕЛЬНОСТИ ФОРСУНОК, а не K. Значение = 33280 × 220 ÷ поток_новых. ЧТО ДЕЛАЕТ: впрыск = ((наполнение × поправки)>>16 × ЭТО)>>16 + мёртвое время (код 8a75-8a8e). Множит ТОЛЬКО длительность, наполнение не трогает — значит НЕ двигает ни оси карт, ни пол/потолок нагрузки, ни пороги. Мёртвое время прибавляется ПОСЛЕ и не масштабируется (проверено замером). Дробь = значение/65536, сток 0.5078. ⚠ Число лежит ВНУТРИ команды LDD по адресу $8A80 (единственное вхождение во всём ПЗУ, проверено). При правке заводского кода в районе расчёта впрыска — перепроверить адрес. ⚠ Пусковой режим этот множитель НЕ покрывает: масштабировать 0x7E20 отдельно."},
 {"a":0x7EEE,"len":2,"kind":"scalar16","conv":"vq16","name":"Шаг лямбда-коррекции, УСТАНОВИВШИЙСЯ режим","note":"сток=15728. Шаг интегратора замкнутого цикла: ALPHA $1431 ∓= (ЭТО × впрыск $1411)>>16, то есть 0.24 от длительности впрыска. Работает В ОБЕ СТОРОНЫ: вычитание на 97e4-97fe, прибавление на 9884-989c. Выбор между этой константой и 0x7F0C делает таймер $1480 (TST $1480): ноль → берётся ЭТА. ⚠ ПРИ СМЕНЕ ФОРСУНОК МЕНЯТЬ: впрыск стал короче в отношение раз — шаг стал мельче во столько же, и контур будет во столько же раз дольше выходить на цель. Новое = 15728 × поток_новых ÷ 220. Обращений во ВСЁМ ПЗУ ровно два (97e4 и 9884), проверено поиском — больше эту константу никто не читает."},
 {"a":0x7F0C,"len":2,"kind":"scalar16","conv":"vq16","name":"Шаг лямбда-коррекции, ПЕРЕХОДНЫЙ режим (таймер $1480)","note":"сток=15728 (в стоке равен 0x7EEE, то есть режимы пока не различаются). Тот же шаг интегратора и так же в ОБЕ стороны (97ee и 988e), но берётся пока тикает таймер $1480. Таймер взводится константой $FF29 при резком изменении (код 9381-9389) и отсчитывается на 9d1d — то есть это шаг в переходном режиме, сразу после броска. Менять вместе с 0x7EEE тем же числом: 15728 × поток_новых ÷ 220. Обращений ровно два (97ee и 988e), проверено поиском."},
 {"a":0x7F2B,"len":2,"kind":"scalar16","conv":"vq16","name":"K — масштаб ШКАЛЫ НАГРУЗКИ (Injector K, НЕ ручка форсунок)","note":"сток=345 (у Z31 607). ЧТО ДЕЛАЕТ: наполнение = расход × K×8 ÷ обороты (код 89b6-89db). Наполнение — это И топливо, И ОСЬ НАГРУЗКИ карт, и вход всех порогов. ⚠ ПРИ СМЕНЕ ФОРСУНОК ЕГО НЕ ТРОГАТЬ — для этого есть множитель 0x0A81. Крутанёшь K — уедет ось карт смеси и угла (0x7AF0/0x7B10), пол и потолок наполнения (0x7E80/0x7E90), амплитуда обогащения ускорения и пороги по наполнению (80/54/95 в коде a075/b30c/b472) — всё это придётся пересчитывать руками. K трогают только когда осознанно меняют саму ШКАЛУ нагрузки."},
