...
PROG = [
    ...
    # --- одноразовая инициализация SCI (только если TE ещё не стоит) ---
    (None,     "LDAAd", 0x11),          # прочитать TRCSR
    (None,     "BITA#", 0x02),          # TE уже включён?
    (None,     "BNE",   "DUMP"),        # да → SCI настроен, init пропустить (RMCR на ходу не трогаем)
    (None,     "LDAA#", 0x06),          # RMCR=0x06: ВНУТРЕННИЙ клок, async 8-бит, E/1024
                                        # (ОЧЕНЬ медленно, бит ~320мкс — против сильной RC-размазки
                                        #  в тапе. E/16=0x04 и E/128=0x05 бились: AE/17 корёжились)
    (None,     "STAAd", 0x10),          # сток ставит 0x0C = ВНЕШНИЙ клок (нужен клок на P22,
                                        # которого нет) — без этого SCI не сдвигает биты
    (None,     "CLRe",  0x001E),        # TRCSR2=0: БЕЗ чётности, 1 стоп → 8N1
                                        # (сток ставил 0x04 = PEN=1 = чётность = 8E1, FTDI на 8N1 читал бы кашу)
    (None,     "OIMd",  (0x02, 0x11)),  # TE=1: включить передатчик SCI (один раз)
