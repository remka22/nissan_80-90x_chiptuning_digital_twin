...
мельче, и контур во столько же раз дольше выходит на цель. Возвращаем прежнюю скорость.
...
GROUPS = [
  ...
  ("🎚 Дроссель (TPS = АЦП ch6): обогащение ускорения + фильтры + пороги", [0x7910, 0x7700, 0x7720, 0x7600, 0x79A0, 0x79B0, 0x79C0, 0x79D0, 0x7691, 0x76CD, 0x7920, 0x7921, 0x7679, 0x767A]),
...
const AX={
  ...
  tps16:['0','32','64','96','128','160','192','224','256','288','320','352','384','416','448','480'],
};
...
function render(){
  ...
  if(cg)cg.addEventListener('click',()=>{
    ...
    document.getElementById('status').textContent=
      ...
      ', пусковое ×'+r.toFixed(3)+', шаг лямбды '+st1+'. K не тронут. Мёртвое время: '+latTxt+'.';
    render();
  });
