...
ADDR_LIST = [
    ...
    0x00F7,           # ДАВЛЕНИЕ в кПа — его считает рутина ДАД ($C700, врезка 8965);
                      # в режиме расходомера рутина до этой строчки не доходит -> мусор
    ...
    # --- РАСХОД, который отдала прослойка ДАД (старший байт $144B) ---
    # С 09.08.26 ДАД врезан на 8965 — на выход таблицы линеаризации, — и отдаёт
    # РАСХОД, а не наполнение. Дальше заводская цепочка сама фильтрует, зажимает и
    # делит на обороты. Логируем старший байт $144B, чтобы видеть, не упёрлись ли в
    # заводские зажимы: нижний 124 (старший байт 0) и верхний 65414 (старший 255).
    # Рабочий диапазон — примерно 1100 на холостом (старший 4) до 30000 на верхах (117).
    0x144B,
]
