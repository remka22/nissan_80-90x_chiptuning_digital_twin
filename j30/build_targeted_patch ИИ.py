...
ADDR_LIST = [
    ...
    0x17E0,           # старший байт указателя карты смеси: $16 = ТЕНЬ, $FD = ПЗУ
]
...
PTR_SS   = 0x17E0                     # указатель карты смеси ($16 = тень, $FD = ПЗУ)
...
for _n, _a, _sz in (("тень", SH_SS, 256), ("кольцо", RING, RING_MASK + 1), ("снимок", SNAP, len(ADDR_LIST)),
                    ("буфер блока", STAGE, BLK_MAX), ("переменные", PTR_SS, 0x1800 - PTR_SS)):
...
assert STAGE + BLK_MAX <= PTR_SS,      "буфер блока залезает в переменные"
