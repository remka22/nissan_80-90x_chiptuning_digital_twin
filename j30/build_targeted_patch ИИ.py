...
ADDR_LIST = [
    ...
    # --- дроссель TPS-производные и коррекции (трассы 01.08.26) ---
    0x14A2,           # отклонение TPS (индекс коррекций; обрыв TPS→44)
    0x14A3,           # скорость TPS вверх = ускорение
    0x14DE, 0x14DF,   # обогащение ускорения (подмешивается в форсунку, 16б)
    0x00B9,           # флаг TPS: бит0x20 = ХХ/WOT/неисправность
    0x00AE,           # бит0x80 = вход в замкнутый цикл
    # --- ДАД: реально выбранные из ОЗУ (пишет рутина C700, сток их не трогает) ---
